...
def get_loader(args):
    ...
    if args.dataset == 'CUB_200_2011':
        ...
        trainset = dogs(root=args.data_root, train=True,  cropped=False,
                        transform=train_transform, download=False)
        testset  = dogs(root=args.data_root, train=False, cropped=False,
        ...
        train_transform = transforms.Compose([
            transforms.Resize((400, 400), _BILINEAR),
    ...
    test_sampler  = (SequentialSampler(testset) if args.local_rank == -1
                     else DistributedSampler(testset))

    # pin_memory / num_workers: args 에서 읽거나 기본값 사용
    _pin    = getattr(args, 'pin_memory',  True)
    _nwork  = getattr(args, 'num_workers', 4)
