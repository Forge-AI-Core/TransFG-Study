...
"""
project_config.py
TransFG Stanford Cars 프로젝트 공통 설정 (Phase 2 - Aircraft→Cars 전이학습)
- 경로 자동 감지 (Stanford_Cars 폴더 구조 처리: parquet 기반)
...
# Stanford_Cars: HuggingFace tanganke/stanford_cars (parquet 형식)
# experiments/stanford_cars/Stanford_Cars/data/{train,test}-*.parquet
DATA_ROOT = os.path.join(PROJECT_ROOT, 'Stanford_Cars')

# ── 가중치 설정 (Phase 2: Aircraft→Cars 전이학습) ─────────────
PRETRAINED_DIR      = os.path.join(PROJECT_ROOT, 'pretrained')
IMAGENET_WEIGHTS    = os.path.join(PRETRAINED_DIR, 'ViT-B_16.npz')
AIRCRAFT_PRETRAINED = os.path.join(PRETRAINED_DIR, 'aircraft_pretrained.bin')

# 기본 가중치: Aircraft 학습 결과 우선 사용 (Phase 2 핵심)
# Aircraft 가중치가 없으면 ImageNet으로 폴백 (baseline 비교용)
PRETRAINED_WEIGHTS = AIRCRAFT_PRETRAINED if os.path.isfile(AIRCRAFT_PRETRAINED) else IMAGENET_WEIGHTS
...
NUM_CLASSES = 196                # Stanford Cars: 196 models (make/model/year)
MODEL_TYPE  = 'ViT-B_16'
IMG_SIZE    = 448
...
def make_args(**overrides) -> argparse.Namespace:
    """
    기존 trainer.py, data_loader_cars.py 등과 호환되는
...
if __name__ == '__main__':
    ...
    print(f"  Aircraft exists   : {os.path.isfile(AIRCRAFT_PRETRAINED)}")
